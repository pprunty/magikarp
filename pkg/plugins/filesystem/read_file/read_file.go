...
func run(ctx context.Context, inMap map[string]any) (*agent.ToolResult, error) {
    ...
    if fileInfo.IsDir() {
        return agent.NewToolResult("read_file", fmt.Sprintf("Path points to a directory, not a file: %s", path), true), nil
    }
    ...
    if in.IncludeStats {
        ...
        if err != nil {
            return agent.NewToolResult("read_file", "Error generating stats JSON", true), nil
        }

        return agent.NewToolResult("read_file", string(statsJSON), false), nil
    }

    // Return just the content for simple requests
    return agent.NewToolResult("read_file", content, false), nil
}
